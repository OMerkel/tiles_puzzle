...
1. Serve `src/` through HTTP(S) (required for service workers and full PWA):
...
See [doc/software_architecture.md](doc/software_architecture.md) for the full
...
- [js/tiles_puzzle.js](js/tiles_puzzle.js) — Main `PuzzleGame` controller:
  game state, lifecycle, celebration animation, and seeded shuffle orchestration
- [js/board.js](js/board.js) — Board sizing, adjacency, valid-move,
  puzzle-naming, and seeded RNG helpers
- [js/input.js](js/input.js) — Keyboard/touch gesture mapping and shortcut
  detection
- [js/storage.js](js/storage.js) — localStorage persistence with
  per-grid-per-date isolation, legacy migration, and settings reset
- [js/view.js](js/view.js) — Tile and board DOM rendering helpers
- [sw.js](sw.js) — Service worker with install and offline cache strategies
- [doc/software_architecture.md](doc/software_architecture.md) — Detailed
